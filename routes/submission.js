...
router.route('/submissions').post(codeValidation(), validate, submitCode) // execute the code.
// .get(isAuthenticated, getSavedSubmissions) // Get all saved submissions.

router.route('/submissions/:id').get(getSubmissionById) // Get a specific submission.
...
router.route('/submissions/save').post(saveSubmission) // Save a submission.
router.route('/submissions/:id/status').get(getSubmissionStatus) // Get the status of a specific submission.
